...
| semaphore | int | 5 | 最大并发数（必须 > 0） |
| timeout | float | 120 | 单页总超时秒数（含重试），5~600 |

### 请求示例

```bash
curl -X POST http://localhost:8000/api/spider/kaogujia \
...
```json
{ "code": 0, "message": "ok", "data": [["第1页记录"], ["第2页记录"]] }
```

> `data` 为二维列表，每个子元素对应一页的记录；个别页失败时仍返回成功页数据，失败详情见服务端日志。
